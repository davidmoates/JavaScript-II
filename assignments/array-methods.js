...
let fullName = [];
const runnerNames = runners.forEach((name) =>
...
let allCaps = runners.map((capNames) => capNames.first_name.toUpperCase());
...
let ticketPriceTotal = runners.reduce((total, donation) => total += donation.donation, 0);
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 Find out how many member donated more then $100; they get free breakfast as part of our gold donor package.
let goldDonor = runners.filter((gold) => gold.donation > 200);
console.log(goldDonor);

// Problem 2 The students that signed up need to be sent a schedule for where to be while the 5k is going on. They will be helping direct people. Get their information base on the .edu email address.
let orgE = runners.filter((orgEmail) => orgEmail.email.endsWith('edu'));
console.log(orgE);

// Problem 3 The director insisted that we read out the ID then the Company then the last name of everyone at the 5k. Get an array of that information. Make sure it is easy to read. The MC has very bad vision.
let comID = runners.map((company) => `${company.id}: ${company.company_name}: ${company.last_name}`);
console.log(comID);
